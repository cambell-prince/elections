...
.directive('breadcrumbs', ['breadcrumbService', '$log', function(breadcrumbService, $log) {
	return {
		restrict : 'A',
		template : '<ul class="breadcrumb"><li ng-repeat="bc in breadcrumbs" ng-class="{active: $last}" ng-switch="$last"><span ng-switch-when="false"><a ng-click="unregisterBreadCrumb( $index )" ng-href="{{bc.href}}">{{bc.label}}</a></span><span ng-switch-default>{{bc.label}}</span></li></ul>',
		replace : true,
